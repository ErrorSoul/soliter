...
- **Мерж в master** разрешён только после: апрув трёх советников (advisor + grok-4.6 + grok-4.5) **И** явный апрув пользователя.
...
- Независимое ревью двумя моделями (grok-4.6 + grok-4.5, дефолтный effort) → скилл `grok-review`.
  Оба id живые, `grok models` проверять перед прогоном. Старые id `grok-build` и
  `grok-composer-2.5-fast` мертвы — падают с "unknown model id" и дают пустой вывод.

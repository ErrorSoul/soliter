...
- **Победа** → `main.script` (`board_cleared` = 27 номиналов + пустые ячейки + 0 драконов; `declare_victory`; точки входа — `card_to_base` и `dragons_collected`) + `tutorial_state.show_victory` (UI poll). Совпадает с `rules.is_goal`.

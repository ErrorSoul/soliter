...
- **Победа** → `main.script` (`board_cleared` = 27 номиналов + пустые ячейки + 0 драконов + **цветок в слоте**; `declare_victory`; точки входа — `card_to_base` и `dragons_collected`) + `tutorial_state.show_victory` (UI poll). С солверным `rules.is_goal` совпадает **по следствию, не по формуле** (у того — пустой tableau + пустые ячейки, без foundation и цветка; см. `architecture.md`). Мест, где ставится WIN, **пять**, и условия разные — полный список с обоснованием в `architecture.md` → «Победа». Главное: `auto_collect_give_up` намеренно слабее (27 номиналов + пустые ячейки, без чистого стола) — запасной выход G2a от софт-лока, см. блок L в `plans/review-fixes.md`; все пять зовут `tutorial_state.request_victory()`, поэтому платформенный `stop` уходит с любой победы.

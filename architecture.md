...
tableau не может прятать цветок, а живую ячейку она проверяет сама. Проверка
живой ячейки в ней осталась и после блока N (L2), хотя авто-финиш теперь ячейки
разгребает: разгребает он только то, что ложится в foundation, а страховка нужна
ровно на случай, когда карту из ячейки положить некуда — цветок, дракон, не тот
номинал. Пятая — про
...
кнопкой сам. Кнопка шлёт `dragons_collected` через 1.5 с после клика, а цветок,
лежавший под собранным драконом, открывается только после дуги (~0.7 с), ждёт
кадр `pending_top_check` и летит ещё ~0.7 с — то есть садится примерно тогда же,
на границе. Проиграв эту гонку, проверка видела `flower_collected = false`,
уходила в ветку «ещё не чисто», и переспросить было некому. Второй путь —
реплей солвера, где последней директивой идёт автоулёт цветка (замер grok-4.6 по
сидам 1..200: из 182 решаемых такой хвост у 3). Точка 6 — единственная точка
победы, достижимая при `auto_collecting == false` и без `card_to_base`.

⚠ **Чего в этом списке НЕТ, хотя я это написал и пришлось убрать.** Сначала здесь
значился третий путь: «цветок припаркован в свободную ячейку и переложен в слот
последним ходом». Ревью блока M разошлось: grok-4.5 показала зондом, что
`flower_collected` приходит РАНЬШЕ `free_cell_changed`, и на такой доске победа
не объявится; grok-4.6 ответила, что доски этой не бывает. Пересчитал сам —
права вторая: `dragons_left == 0` требует, чтобы все 12 драконов лежали в
запечатанных ячейках, а это ВСЕ ТРИ ячейки. Пока цветок занимает одну, третью
масть собрать нечем; значит цветок уносят в слот раньше, а последним событием
партии оказывается `dragons_collected`. Порядок сообщений, который нашла 4.5,
реален — просто разыгрывается на недостижимой доске. Держать это в голове стоит:
если ячеек когда-нибудь станет четыре, дыра станет достижимой.
...
| `main.free_cell_state` | `init`; `free_cell_changed` (C4); локальная очистка в `auto_finish_step` (L2) | снапшот + **релизное условие победы** (`live_cells`/`board_cleared`) + **релизный** `find_next_auto_card`/`can_auto_finish` (L2: авто-финиш забирает номинал из живой ячейки) | остаётся — победа читает его напрямую |
| `main.flower_collected` | `init`; `flower_collected` (C5) | снапшот + **релизное условие победы** (`board_cleared`, блок L1) + сама точка победы 6 (блок M2) | остаётся — без него победа недостижима |

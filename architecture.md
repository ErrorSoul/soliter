...
живой карты в свободных ячейках **И** ни одного несобранного дракона. Это ровно то
же, что считает целью солвер (`rules.lua is_goal`); до 2026-08-21 игра объявляла
победу по одному счётчику `base_cards_count >= 27` и расходилась со своим эталоном
(замер: в 26% решаемых раздач 27-я карта садится, когда на столе ещё 4 дракона).
